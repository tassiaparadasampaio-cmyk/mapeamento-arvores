...
# --- CONFIGURAÇÃO ---
#  URL CSV publicada da planilha Google Sheets
CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vR2Xqpbb7dcg2ZBFKrL5IClkmnHCg1kzCmwgbDdpj8xBe1U1yBSIa3oINkOZFSfdjd5PsvKFOkk62oW/pub?output=csv" 
LAYER_NAME = "Ficha_Avaliacao_Arborizacao"
CRS_WKT = "EPSG:4326"
UPDATE_INTERVAL_MS = 60000 # 60 segundos
# --- FIM DA CONFIGURAÇÃO ---
...
def update_layer_from_csv_url():
    ...
    try:
        ...
        print(f"❌ ERRO: Falha ao baixar o CSV. Verifique a URL: {CSV_URL}")
        return
            ...
            for row in reader:
                ...
                if fixed_lat and fixed_lon:
                    ...
                    if valid_rows == 1:
                        ...
                        print(f"   Espécie Científica: {row.get('Especie_Cientifica', 'N/A')}")
                        print(f"   DAP: {row.get('DAP', 'N/A')}")
                        print(f"   Conflito: {row.get('Rede_Eletrica', 'N/A')}")
                else:
                    invalid_rows += 1
                    # Não imprime todas as linhas ignoradas para evitar poluição do console
                    if invalid_rows < 5:
    ...
    # O URI precisa saber que os campos X e Y são Longitude e Latitude, respectivamente
